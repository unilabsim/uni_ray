...
`tests/` or reproducible via `benchmarks/bench_mjbatch.py`. The mjwarp (#300)
and motrix (#301) profiles — the same descriptor and caster sourced from a
`mujoco_warp.Model` or a `motrixsim.SceneModel` — are documented in
